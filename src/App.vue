<template>
  <div class="min-h-screen flex flex-col justify-center items-center">
    <header class="flex flex-col w-full container">
      <h1 class="text-6xl font-semibold text-gray-800 text-center">
        My To do List
      </h1>
      <input 
    ...
    <main class="container w-full mt-8 space-y-8">
      <section class="space-y-4"  v-if="pendingTodos.length > 0">
        <h3 class="text-3xl font-light text-red-900">
        ...
        <ul class="space-y-4">
          <li 
            ...
            :key="todo.id"
            class="bg-white rounded-xl text-2xl py-2 px-4 font-light text-green-900 text-center hover:text-white hover:bg-green-800 cursor-pointer transition-colors"
      ...
      <section class="space-y-4" v-if="completedTodos.length > 0">
        <h3 class="text-3xl font-light text-green-900">
        ...
        <ul class="space-y-4">
          <li 
            ...
            :key="todo.id"
            class="bg-white rounded-xl text-2xl py-2 px-4 font-light text-green-900 text-center hover:text-white hover:bg-red-800 cursor-pointer transition-colors"
            @click="changeStatus(todo.id)"
            >
            {{ todo.text }}
          </li>
